...
export function SettingsScreen(): React.JSX.Element {
  ...
  const [currentLedgerId, setCurrentLedgerIdState] = useState<string | undefined>(
    () => getCurrentLedgerId(),
  );
  ...
  const handleDeleteLedger = useCallback(
    (ledger: Ledger) => {
      ...
      Alert.alert(
        '장부 삭제',
        `"${ledger.name}" 장부와 모든 거래를 삭제하시겠습니까?`,
        [
          {text: '취소', style: 'cancel'},
          {
            text: '삭제',
            ...
            <View key={ledger.id} style={[styles.ledgerItem, {borderBottomColor: theme.border}]}>
              ...
              >
                ...
                <Text
                  style={[
                    styles.ledgerName,
                    {color: ledger.id === currentLedgerId ? theme.primary : theme.text},
                  ]}
                >
                  {ledger.name}
                </Text>
