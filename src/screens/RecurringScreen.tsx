    ...
    <Swipeable ref={swipeRef} renderRightActions={renderRight} overshootRight={false} friction={2}>
      <Pressable onPress={() => onEdit(item)}>
        <View
          style={[
            styles.item,
            { backgroundColor: theme.card, borderBottomColor: theme.rule, opacity: active ? 1 : 0.4 }
          ]}
        >
...
export function RecurringScreen(): React.JSX.Element {
  ...
  const expenseTotal = React.useMemo(
    () => rules.filter(r => r.type === "expense" && r.is_active).reduce((s, r) => s + r.amount, 0),
    [rules]
  );
  const incomeTotal = React.useMemo(
    () => rules.filter(r => r.type === "income" && r.is_active).reduce((s, r) => s + r.amount, 0),
    [rules]
  );
      ...
      >
        ...
        >
          ...
          <View style={[styles.typeToggle, { borderColor: theme.rule }]}>
            ...
            >
              <Text style={[styles.typeBtnText, { color: formType === "expense" ? theme.card : theme.mute1 }]}>
                지출
              </Text>
            </Pressable>
            <Pressable
            ...
            >
              <Text style={[styles.typeBtnText, { color: formType === "income" ? theme.card : theme.mute1 }]}>
                수입
              </Text>
            </Pressable>
          </View>
          ...
          <View style={styles.chips}>
            {formCategories.map(c => (
              <Pressable
                ...
                style={[
                  styles.chip,
                  { borderColor: c === category ? theme.ink : theme.rule },
                  c === category && { backgroundColor: theme.ink }
                ]}
          ...
          <View style={styles.chips}>
            {allPaymentMethods.map(m => (
              <Pressable
                ...
                style={[
                  styles.chip,
                  { borderColor: m === payment ? theme.ink : theme.rule },
                  m === payment && { backgroundColor: theme.ink }
                ]}
