...
export function createRecurringRule(params: {
  ...
  category: string; payment_method: string; memo: string | null; day_of_month: number;
}): RecurringRule {
  const db = getDB();
...
export function updateRecurringRule(id: string, params: {
  ...
  payment_method: string; memo: string | null; day_of_month: number; is_active: boolean;
}): void {
  const db = getDB();
  db.executeSync(
    `UPDATE recurring_rules SET type=?, amount=?, category=?, payment_method=?, memo=?, day_of_month=?, is_active=? WHERE id=?`,
    [params.type, params.amount, params.category, params.payment_method, params.memo, params.day_of_month, params.is_active ? 1 : 0, id],
  );
}
      ...
      if (targetDate > lastCheck && targetDate <= today) {
        ...
        if ((existing.rows?.length ?? 0) === 0) {
          ...
          db.executeSync(
            ...
            [id, ledgerId, rule.type, rule.amount, rule.category, rule.payment_method, rule.memo, targetDate, Date.now()],
          );
          created++;
        }
      }
