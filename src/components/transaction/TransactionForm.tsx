...
export function TransactionForm({
  ...
  const [date, setDate] = useState(editTransaction?.date ?? initialDate ?? todayString());
  const [category, setCategory] = useState(editTransaction?.category ?? "");
  const [paymentMethod, setPaymentMethod] = useState(editTransaction?.payment_method ?? allPaymentMethods[0] ?? "");
  ...
  return (
    ...
    >
      {/* Title */}
      <Text style={[styles.title, { color: theme.text }]}>{editTransaction ? "거래 수정" : "거래 추가"}</Text>
      ...
      <View style={styles.section}>
        ...
        <BottomSheetTextInput
          style={[
            styles.amountInput,
            { color: amountColor, borderColor: theme.border, backgroundColor: theme.background }
          ]}
          value={amountText}
