  ...
  if (data.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text style={[styles.emptyText, { color: theme.textSecondary }]}>지출 데이터가 없습니다</Text>
      </View>
    );
  }
        ...
        <PieChart
          ...
          centerLabelComponent={() => (
            <View style={styles.centerLabel}>
              <Text style={[styles.centerAmount, { color: theme.text }]}>{formatAmount(total)}</Text>
      ...
      <View style={styles.legend}>
        {data.map((item, index) => (
          <View key={item.category} style={styles.legendItem}>
            ...
            <Text style={[styles.legendAmount, { color: theme.textSecondary }]}>{formatAmount(item.total)}원</Text>
          </View>
        ))}
      </View>
    </View>
