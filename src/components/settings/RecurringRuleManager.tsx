...
export function RecurringRuleManager(): React.JSX.Element {
  ...
  const [rules, setRules] = useState<RecurringRule[]>(() =>
    ledgerId ? getRecurringRules(ledgerId) : [],
  );
  ...
  const handleAdd = useCallback(() => {
    ...
    setFormPayment('');
    setShowForm(false);
  }, [formType, formDay, formAmount, formCategory, formMemo, formPayment, ledgerId, categories, paymentMethods, refreshRules]);
  ...
  return (
    <View>
        ...
        <View key={rule.id} style={[styles.item, {borderBottomColor: theme.border}]}>
          <View style={styles.itemLeft}>
            ...
            <Text style={[styles.itemSub, {color: theme.textSecondary}]}>
              {rule.type === 'income' ? '+' : '-'}{formatAmount(rule.amount)}원
              {rule.memo ? ` · ${rule.memo}` : ''}
            </Text>
          </View>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <Text style={[styles.toggleFormText, {color: theme.primary}]}>
          {showForm ? '취소' : '+ 반복 거래 추가'}
        </Text>
      </TouchableOpacity>
