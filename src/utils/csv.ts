...
export interface CsvRecurringRow {
  ...
  payment_method: string; memo: string | null; day_of_month: number;
  is_active: boolean;
}
...
function recurringRulesToCsv(rules: RecurringRule[]): string {
  const rows = rules.map(r => ({
    '유형': TYPE_MAP_REVERSE[r.type], '금액': r.amount, '카테고리': r.category,
    '결제수단': r.payment_method, '메모': r.memo ?? '', '반복일': r.day_of_month,
    '활성': r.is_active ? 'Y' : 'N',
  }));
  return Papa.unparse(rows, {newline: '\n'});
}
