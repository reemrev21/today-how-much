...
const DEFAULT_CATEGORIES = [
  '식비', '교통비', '주거비', '통신비', '의료비', '문화생활',
  '쇼핑', '교육', '경조사', '기타지출', '월급', '부수입', '용돈', '기타수입',
];
...
export function getCategories(): string[] {
  ...
  if (!raw) { storage.set(KEYS.CATEGORIES, JSON.stringify(DEFAULT_CATEGORIES)); return DEFAULT_CATEGORIES; }
...
export function getPaymentMethods(): string[] {
  ...
  if (!raw) { storage.set(KEYS.PAYMENT_METHODS, JSON.stringify(DEFAULT_PAYMENT_METHODS)); return DEFAULT_PAYMENT_METHODS; }
  return JSON.parse(raw);
}
